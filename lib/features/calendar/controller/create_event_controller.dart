...
//           ? null
//           : descriptionController.text.trim(),
//       eventTypeId: getEventTypeId(eventType.value)!,
//     );
...
//     if (result['success'] == true) {
//       // ✅ refresh calendar
//       Get.find<CalendarController>().refreshEvents();
...
/// Create Event Controller
/// Handles create event flow safely (no controller dispose crash)
class CreateEventController extends GetxController {
  // Storage & Services
  final storage = GetStorage();
  final AuthService _authService = AuthService();
  ...
  final RxString endTime = ''.obs;
  final RxString eventType = ''.obs;
  final RxString status = 'confirmed'.obs;
  ...
  /// NOTE: backend currently accepts same event_type_id
  int getEventTypeId(String _) => 1;
  ...
  /// DO NOT dispose controllers manually
  /// GetX handles lifecycle
  @override
  void onClose() {
    super.onClose();
  }
...
String formatTime(String time) {
  ...
  bool validateForm() {
    if (titleController.text.trim().isEmpty) {
      _error('Please enter event title');
      return false;
    }
    ...
    if (startTime.value.isEmpty || endTime.value.isEmpty) {
      _error('Please select start and end time');
      return false;
    }

    final start = _toMinutes(startTime.value);
    final end = _toMinutes(endTime.value);
  ...
  Future<void> handleSubmit() async {
    ...
    try {
      ...
      final result = await _authService.createEvent(
        title: titleController.text.trim(),
        date: dateStr,
        startTime: '$dateStr ${startTime.value}:00',
        endTime: '$dateStr ${endTime.value}:00',
        description: descriptionController.text.trim().isEmpty
            ? null
            : descriptionController.text.trim(),
        eventTypeId: getEventTypeId(eventType.value),
      ...
      if (result['success'] == true) {
        ...
        _error(result['message'] ?? 'Failed to create event');
      }
    } catch (e) {
      isLoading.value = false;
  ...
  // void setStartTime(String time) => startTime.value = time;
  // void setEndTime(String time) => endTime.value = time;
  // void setEventType(String type) => eventType.value = type;
